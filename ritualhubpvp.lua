-- ============================================================
-- RITUAL HUB VERSION 12.5 | BLACK & GOLD | FOV AIMBOT
-- ============================================================
...
if isSpanish then
    popContent.Text = "Sanguine Z sin cooldown, Aimbot FOV, Hitbox/Stun (Beta), Macro Config, Wallpapers, Songs y más.\n\n🟣 ¡El script ahora es SIN KEY! Solo presiona Empezar."
else
    popContent.Text = "Sanguine Z no cooldown, FOV Aimbot, Hitbox/Stun (Beta), Macro Config, Wallpapers, Songs & more.\n\n🟣 The script is now KEYLESS! Just click Get Started."
end
...
_G.G_ESP_HighlightColor = "FFD700" -- gold
...
-- ============================================================
-- METAMETHODS (Silent Aim + Soru Aimbot) + FOV Line
-- ============================================================
-- FOV Circle and Targeting Line
local FOVCircle = nil
...
pcall(function()
    if Drawing and Drawing.new then
        ...
        TargetLine.Visible = _G.G_SilentAimShowLine or false
...
-- Helper to check if a target is within FOV (screen coordinates)
function IsTargetInFOV(targetPart)
    ...
    return dist <= _G.G_SilentAimFOV
end

-- Override GetClosestTargetToCenter with FOV check
local originalGetClosest = GetClosestTargetToCenter
GetClosestTargetToCenter = function()
    local target = originalGetClosest()
...
-- Update line and FOV circle each frame
RunService.RenderStepped:Connect(function()
    pcall(function()
        local cam = workspace.CurrentCamera
        ...
        if TargetLine then
            ...
            if targetPart and targetPart:IsA("BasePart") and _G.G_SilentAimShowLine then
                local screenPos, onScreen = cam:WorldToViewportPoint(targetPart.Position)
                if onScreen then
...
-- Update target each tick (reuse existing logic)
RunService.Heartbeat:Connect(function()
    currentSilentAimTarget = GetClosestTargetToCenter()
end)

-- Hook metamethods (keep original sacred logic, just use our FOV-filtered target)
local oldIndex = nil
...
TRANSLATIONS = {
    EN = {
        ...
        ["Show Targeting Line"] = "Show Targeting Line",
        -- Buttons & Labels
    ...
    ES = {
        ...
        ["Show Targeting Line"] = "Mostrar Línea de Objetivo",
        -- Buttons & Labels
...
-- COMBAT MAIN TAB
do
local c1 = createModuleCard("Aimbot Modules", 320, CombatPage)
...
addToggleElement(c1, "Show Targeting Line", _G.G_SilentAimShowLine or false, 96, function(v) 
    _G.G_SilentAimShowLine = v
    if TargetLine then TargetLine.Visible = v end
end, "ShowLine")

addToggleElement(c1, "Aimbot M1 (Dragon Gun) ⚠️ BAN RISK", _G.G_DragonGunM1, 120, function(v) 
    _G.G_DragonGunM1 = v
    UpdateDragonButton() 
end, "DragonM1")

local setTeamCheckState
local setTargetPlayersState = addToggleElement(c1, "Target Players", _G.G_SilentAimTargetPlayers, 144, function(v) 
...
aimbotTargetBtn.Position = UDim2.new(0, 6, 0, 168)
...
setTargetMobsState = addToggleElement(c1, "Target NPCs", _G.G_SilentAimTargetMobs, 192, function(v) 
    _G.G_SilentAimTargetMobs = v
end, "TargetMobs")

setTeamCheckState = addToggleElement(c1, "Team Check", _G.G_SilentAimTeamCheck, 216, function(v) 
    _G.G_SilentAimTeamCheck = v 
end, "TeamCheck")

addToggleElement(c1, "Ignore Safe Zone (No SafeZone)", _G.G_AimbotSafeZoneCheck, 240, function(v) _G.G_AimbotSafeZoneCheck = v end, "AimbotSafeZone")
addToggleElement(c1, "Ignore PvP OFF Players", _G.G_AimbotPvPCheck, 264, function(v) _G.G_AimbotPvPCheck = v end, "AimbotPvP")
addToggleElement(c1, "Target Rainbow Body ESP", _G.G_TargetRainbowBodyESP, 288, function(v) _G.G_TargetRainbowBodyESP = v end, "RainbowBodyESP")
addStepper(c1, "Aimbot Max Dist:", 312, 100, 5000, 250, function() return maxRange end, function(v) maxRange = v end, "st")
...
print("✅ RITUAL HUB v12.5 LOADED - BLACK & GOLD THEME - FOV AIMBOT ACTIVE")
